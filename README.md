...
- (опционально) WeasyPrint для PDF-генерации
...
5. Показать печать справки по студенту и (при установке WeasyPrint) загрузку PDF.

## PDF
- Основной способ: открыть страницу отчета и выбрать «Печать в PDF» в браузере.
- Дополнительно: endpoint `/reports/transcript/<id>/pdf` использует WeasyPrint, если пакет установлен.
